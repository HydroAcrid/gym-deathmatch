...
export async function POST(req: Request) {
	...
	try {
		...
		const result = await CommentaryJobService.runWeekly({ lobbyId, processQueue });
		...
		console.error("weekly commentary cron failed", err);
		return NextResponse.json({ error: "WEEKLY_COMMENTARY_FAILED" }, { status: 500 });
	}
}
