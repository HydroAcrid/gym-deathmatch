				...
				for (const p of data.lobby.players) {
					...
					for (const e of (p.events ?? [])) {
						items.push({
							...
							when: e.weekStart,
							player: p.name,
							line: e.met ? `Weekly target met (${e.count})` : `Missed target (${e.count}) — a heart trembles`,
							icon: e.met ? "✅" : "⚠️"
						});
					}
	...
	return (
		<div className="mx-auto max-w-6xl">
			<div className="paper-card paper-grain ink-edge p-5 mb-6 border-b-4" style={{ borderColor: "#E1542A" }}>
				...
				<div className="text-deepBrown/70 text-xs">Live feed of workouts and highlights • Lobby: {lobbyId}</div>
			</div>
